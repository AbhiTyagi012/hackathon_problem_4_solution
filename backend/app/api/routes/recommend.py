...
def recommend_home(req: HomeRequest, svc: RecommendationService = Depends(get_recommendation_service)):
    return svc.home(req.profile, req.shopper_id)
...
def recommend_search(req: SearchRequest, svc: RecommendationService = Depends(get_recommendation_service)):
    return svc.search(req.profile, req.shopper_id, req.search_query, req.search_category)
...
def recommend_purchase(req: PurchaseRequest, svc: RecommendationService = Depends(get_recommendation_service)):
    return svc.purchase(req.profile, req.shopper_id, req.purchased_product_id)
...
def recommend_similar(req: SimilarRequest, svc: RecommendationService = Depends(get_recommendation_service)):
    """Purchase-history rail: embeddings + cosine similarity, not rule-based."""
    return svc.similar_to_purchases(req.shopper_id)
